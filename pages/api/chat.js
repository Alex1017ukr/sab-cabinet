...
export default async function handler(req, res) {
  ...
  const SYS = 'Ти AI-асистент діловодця (Шершун Олександр Петрович) CAБ 2АДн ЗСУ. Командир: капітан Мураховський Владислав Григорович. Відповідай УКРАЇНСЬКОЮ мовою коротко.';
  try {
    let reply = '';
    if (ai === 'claude') {
      const k = process.env.ANTHROPIC_API_KEY;
      if (!k) throw new Error('Немає ANTHROPIC_API_KEY');
      ...
      reply = d.content?.[0]?.text||'Порожня';
    } else if (ai === 'gemini') {
      const k = process.env.GEMINI_API_KEY;
      if (!k) throw new Error('Немає GEMINI_API_KEY');
      const genAI = new GoogleGenerativeAI(k);
      const model = genAI.getGenerativeModel({model:'gemini-1.5-flash',systemInstruction:SYS});
      const chat = model.startChat({ history: [] });
      const result = await chat.sendMessage(message);
      reply = result.response.text();
    } else if (ai === 'gpt') {
      const k = process.env.OPENAI_API_KEY;
      if (!k) throw new Error('Немає OPENAI_API_KEY');
      ...
      reply = d.choices?.[0]?.message?.content||'Порожня';
