  ...
  try {
    ...
    if (ai === 'claude') {
      ...
      const resp = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({ model: 'claude-haiku-4-5-20251001', max_tokens: 1024, system: SYSTEM, messages: msgs }),
      });
      const data = await resp.json();
      if (!resp.ok) throw new Error(data.error?.message || 'Claude error');
      reply = data.content?.[0]?.text || 'Порожня відповідь';
      ...
      reply = data.candidates?.[0]?.content?.parts?.[0]?.text || 'Порожня відповідь';
      ...
      const data = await resp.json();
      if (!resp.ok) throw new Error(data.error?.message || 'GPT error');
      reply = data.choices?.[0]?.message?.content || 'Порожня відповідь';
