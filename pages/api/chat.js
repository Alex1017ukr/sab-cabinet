...
export default async function handler(req, res) {
  ...
  const SYSTEM = 'Ти AI-асистент діловодця (Шершун Олександр Петрович) самохідної артилерійської батареї 2-го артилерійського дивізіону (САБ 2АДн) ЗСУ. Командир: капітан Мураховський Владислав Григорович. Відповідай УКРАЇНСЬКОЮ мовою коротко і по суті.';
  try {
    let reply = '';
    if (ai === 'claude') {
      const key = process.env.ANTHROPIC_API_KEY;
      if (!key) throw new Error('Claude API ключ не налаштований');
      ...
      if (!key) throw new Error('Gemini API ключ не налаштований');
      const genAI = new GoogleGenerativeAI(key);
      const model = genAI.getGenerativeModel({ model: 'gemini-1.5-flash', systemInstruction: SYSTEM });
      const chatHist = history.filter(m=>m.role!=='system').slice(-10).map(m=>({role:m.role==='user'?'user':'model',parts:[{text:m.text}]}));
      const chat = model.startChat({ history: chatHist });
      const result = await chat.sendMessage(message);
      reply = result.response.text();
    } else if (ai === 'gpt') {
      const key = process.env.OPENAI_API_KEY;
      if (!key) throw new Error('GPT API ключ не налаштований');
    ...
    } else { throw new Error('Невідомий AI: '+ai); }
    return res.json({reply});
  } catch(e) { console.error('[chat]',e.message); return res.status(500).json({error:e.message}); }
}